...
def packet_callback(packet):
    ...
    if UDP in packet:
        ...
        if channels_open:
            data = bytes.fromhex(
                dict_data["rcv"]["heartbeat"][0]
            )

            send_udp_data(data=data, target_ip=source_ip, source_ip=target_ip, port=cmd_port)
        ...
        if packet[IP].dst == target_ip and packet[UDP].dport == cmd_port:
            ...
            print("1 " + "-" * 50)
            print(f"Dados enviados de {packet[IP].src} para {packet[IP].dst} na porta {packet[UDP].dport}")
